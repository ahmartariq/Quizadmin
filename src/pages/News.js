...
export default function News() {
  ...
  const currentData = filteredNews.slice(indexOfFirstData, indexOfLastData)
  ...
  useEffect(() => {
    setFilteredNews(filterbySearch(news, filter));
  },[filter
  ]);
  ...
  return (
    <div className="w-full min-h-screen sm:max-w-screen-2xl px-6 sm:px-8 xl:px-0 xl:py-6 sm:mx-auto">
      <div className="">
        ...
        <Pagination perPage={perPage} totalData={filteredNews.length} paginate={paginate} currentPage={currentPage}/>
    </div>
  );
}
